...
JSON_PRETTY_PATH = "./v2/latest_daily_report_pretty_print.json"
JSON_PATH = "./v2/latest_daily_report.json"

README_PATH = "./README.md"
META_FILE = "./v2/metadata.json"
META_PRETTY_FILE = "./v2/metadata_pretty_print.json"

DOWNLOAD_PATH = "./v2/csse_covid_19_data/"
...
def dumpCsvToJson(CSV_FILE):
    with open(CSV_FILE, "r") as f:
        reader = csv.reader(f)
        next(reader)
        headers = ["FIPS", "Admin2", "Province/State", "Country/Region", "Last Update", "Latitude", "Longitude",
...
def dumpMetaData(CSV_FILE):
    ...
    csvData.columns = ["FIPS", "Admin2", "Province/State", "Country/Region", "Last Update", "Latitude", "Longitude",
                   "Confirmed", "Deaths", "Recovered", "Active", "Combined_Key"]
    f = csvData.copy()
...
def dumpCsvToJsonPrettyPrint(CSV_FILE):
    ...
    with open(CSV_FILE, "r") as f:
        ...
        # headers = next(reader)
        next(reader)
        headers = ["FIPS","Admin2","Province/State","Country/Region","Last Update","Latitude","Longitude","Confirmed","Deaths","Recovered","Active","Combined_Key"]
        reader = csv.DictReader(f, headers)
...
def dumpTimeSeries(CSV_FILE):
    ...
    with open('./v2/{}.json'.format(filename), "w") as f:
        f.write(json.dumps(results))
    with open('./v2/{}_pretty_print.json'.format(filename), "w") as f:
...
def dumpRecordData(CSV_FILE):
    ...
    my_data["Last_Update"] = pd.to_datetime(my_data["Last_Update"])
    agg = pd.DataFrame(columns=my_data.columns)
    agg =agg.append(pd.Series(), ignore_index=True)
    agg.loc[0]["Country_Region"] = "!summary"


    operations = {'Province_State': 'nunique',
         'Lat': 'mean',
         'Long_': 'mean',
         'Last_Update': 'max',
                  'Combined_Key': 'nunique',
         }



    for i in my_data.columns[7:11]:
        agg[i].loc[0] = my_data[i].sum()
        operations[i] = "sum"
    agg.loc[0]["Province_State"] = my_data["Province_State"].unique().shape[0]
    agg.loc[0]["Last_Update"] = my_data["Last_Update"].max()

    if  "Lat" in my_data.columns:
        agg.loc[0]["Lat"] = my_data["Lat"].mean()
        agg.loc[0]["Long_"] = my_data["Long_"].mean()
    if "Lat" in my_data.columns:
        agg.loc[0]["Lat"] = my_data["Lat"].mean()
        agg.loc[0]["Long"] = my_data["Long"].mean()
    agg.loc[0]["Last_Update"] = my_data["Last_Update"].max()
    my_data.loc[-1] =agg.loc[0]
    my_data["Last_Update"] = my_data["Last_Update"].dt.strftime('%Y-%m-%d %H:%M:%S')
    ...
    g = my_data.groupby(['Country_Region'])
    aggra = g.agg(operations)

    aggra["Province_State"] = aggra["Province_State"].astype(np.int32)
    results = {}
    for key, df_gb in g:
        agg_dict = aggra.loc[str(key)].to_dict()
        agg_dict["Province_State"] = "None"


        records = df_gb.to_dict('records')
        data = {}
        for i in records:
            data_label = i['Province_State']
            if data_label == "":
                data_label = "*"
            data[data_label] = i
        results[str(key)] = {
...
def runTest():
    CSV_PATH = "csse_covid_19_data/csse_covid_19_daily_reports.csv"
